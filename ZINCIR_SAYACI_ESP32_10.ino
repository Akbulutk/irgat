...
int ENABLE_DEMO = 0;                // Set to 1 to enable Demo Mode with up/down counter
int SAFETY_STOP = 0;                // Defines safety stop for chain up. Stops defined number of events before reaching zero
int MAX_CHAIN_LENGTH = 75;          // Define maximum chan length. Relay off after the value is reached
int Max_Zincir = 75;
int Min_Zincir = 0;
int Sayac = 0;
int Sayacold = 0 ;
...
const char *ssid = "IRGAT";     // Set WLAN name
const char *password = "12346789";  // Set password
...
#define Chain_Up_Pin 16              // GPIO pin 14 for Chain Up Relay
#define Chain_Down_Pin 17            // GPIO pin 14 for Chain Down Relay
...
void setup() {
  ...
  digitalWrite(Chain_Up_Pin, HIGH );         // Relay off
  digitalWrite(Chain_Down_Pin, HIGH );       // Relay off
  ...
  server.on("/ns", Event_NoSensor);
  server.on("/ys", Event_YesSensor);
  server.on("/10m", Event_10m);
  server.on("/eksi10m", Event_eksi10m);
  server.on("/75m", Event_75m);
...
void Event_NoSensor() {                          // Handle UP request
 ENABLE_DEMO = 1;
 SAFETY_STOP = -10;  
Serial.println (ENABLE_DEMO);
 }

void Event_YesSensor() {                          // Handle UP request
 ENABLE_DEMO = 0;
 SAFETY_STOP = 0;  
 Serial.println (ENABLE_DEMO);
...
void Event_10m() {        
  ...
  if (Sayacold +10 < Max_Zincir){
  Sayacold = Sayacold +10;
  MAX_CHAIN_LENGTH =  Sayacold ;}
  else {MAX_CHAIN_LENGTH = Max_Zincir;}
  Serial.println(MAX_CHAIN_LENGTH);
  }
...
void Event_eksi10m() {        
  ...
  if (Sayacold -10 > SAFETY_STOP){
  Sayacold = Sayacold -10;
  MAX_CHAIN_LENGTH =  Sayacold ;}
  else {MAX_CHAIN_LENGTH = Min_Zincir;}
  Serial.println(MAX_CHAIN_LENGTH);
  }
...
void Event_Index() {                         // If "http://<ip address>/" requested
  server.send(200, "text/html", index_html);  // Send Index Website
}
...
void loop() {
  ...
  Sayac = (abs(ChainCounter) * Chain_Calibration_Value);  
  ...
  if ( ( millis() > Watchdog_Timer + 1000 ) ||                     // Check HTTP connnection
    ...
    digitalWrite(Chain_Up_Pin, HIGH );                              // Relay off after 1 second inactivity
    digitalWrite(Chain_Down_Pin, HIGH );
    OnOff = 0;
  }
